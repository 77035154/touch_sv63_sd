...
- Manual selection
  ...
  - For RMI printf, first enter '*' then enter 'dc'
...
- Single-line commands
  ...
  - Run SynaToolbox and specify tool selection and parameters as single-line command
    ...
    SynaToolbox rmi_printf [parameters...]
...
RMI Printf (rmi_printf)
-----------------------
Parameters
[-h {hex file}] - Name of hex file
[-t {timeout value}] - Timeout value in seconds
[-b] - Enable blocking mode
[-s] - Enable time stamps

Parameter usage examples
- Run printf in blocking mode with timeout of 5s based on PR1234567.hex
   -h PR1234567.hex -t 5 -b
- Run printf in non-blocking mode with time stamps and no timeout based on PR1234567.hex
   -h PR1234567.hex -s



RAM Backdoor Access (backdoor_access)
-------------------------------------
Parameters
[-a {address in hex}] - Start word address (16-bit) for doing reading/writing
[-l {length to read}] - Length in words to read
[-d {data to write}] - Data words to write
[-r] - Read from start address for number of words set with -l parameter
[-w] - Write data words set with -d parameter to start address
[-x] - Do XOR when writing data words
[-o] - Set offset of F81_RMI_DATA2 register from F$81 data register base address
[-c] - Use /dev/rmi0 character device node

Parameter usage examples
- Use /dev/rmi0 to read five words of data starting from word address 0x048a
   -a 0x048a -l 5 -r -c
- Write 0x1122 0x3344 0x5566 to word address 0x048a starting with 0x1122
   -a 0x048a -d 0x112233445566 -w
- Write 0x55aa to word address 0x01c0 using XOR when offset of F81_RMI_DATA2 register from F$81 data register base address is 2
